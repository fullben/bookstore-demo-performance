...
public interface ActionExecutor {
  ...
  void submit(QueryAction action);

  void remove(QueryAction action);

  void submit(ClearAction action);

  List<QueryAction> submittedQueries();

  void clearQueue();

  void schedule(ClearAction action, int queryInterval);

  void unschedule(ClearAction action);

  static ActionExecutor New(final ExecutionCallback callback) {
    return new Default(callback);
  }

  class Default implements ActionExecutor, Runnable {
    static final class Entry {
      final Action action;

      Entry(final Action action) {
        super();
        this.action = action;
      }
    }
    ...
    Default(final ExecutionCallback callback) {
      super();
    ...
    @Override
    public void submit(final QueryAction action) {
    ...
    @Override
    public void remove(final QueryAction action) {
      if (!this.shutdown.get()) {
        synchronized (this.queue) {
    ...
    @Override
    public void submit(final ClearAction action) {
      if (!this.shutdown.get()) {
        synchronized (this.queue) {
    ...
    @Override
    public void schedule(final ClearAction action, final int queryInterval) {
    ...
    @Override
    public void unschedule(final ClearAction action) {
      synchronized (this.clearSchedules) {
    ...
    public void run() {
      while (true) {
        final Action action;
        synchronized (this.queue) {
          action = this.queue.isEmpty() ? null : this.queue.removeFirst().action;
        }
    ...
    private void execute(final Action action) {
      ...
      if (action instanceof QueryAction) {
        ...
        synchronized (this.clearSchedules) {
          intervalClearActions =
              this.clearSchedules.entrySet().stream()
    ...
    private void executeAndNotify(final Action action) {
