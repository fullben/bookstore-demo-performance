...
public interface ComparingStopwatch {
  ...
  ComparingStopwatch restart();

  ComparingStopwatch stop();

  ComparingStopwatch logResult();

  static ComparingStopwatch Start(final String first, final String second) {
    return new Default(first, second);
  }

  class Default implements ComparingStopwatch, HasLogger {
    ...
    Default(final String first, final String second) {
      this.first = first;
      this.second = second;
    ...
    public ComparingStopwatch logResult() {
      final String firstSeconds =
          String.format(
              "%1$12s",
              this.decimalFormat(4).format(this.divide(this.firstDuration, nanosPerSecond)));
      final String secondSeconds =
          String.format(
              "%1$12s",
              this.decimalFormat(4).format(this.divide(this.secondDuration, nanosPerSecond)));

      final String firstMillis =
          String.format(
                  "%1$10s",
                  this.decimalFormat(2).format(this.divide(this.firstDuration, nanosPerMilli)))
              + "  ";
      final String secondMillis =
          String.format(
              "%1$10s",
              this.decimalFormat(2).format(this.divide(this.secondDuration, nanosPerMilli)));

      final String factor =
          this.decimalFormat(2).format(this.divide(this.secondDuration, this.firstDuration));

      this.logger().info(StringUtils.repeat('-', 35 + this.first.length() + this.second.length()));
      this.logger()
          .info(
              "        |"
                  + StringUtils.repeat(' ', 13 - this.first.length())
                  + this.first
                  + " | "
                  + StringUtils.repeat(' ', 12 - this.second.length())
                  + this.second
                  + " | Factor");
      this.logger().info(StringUtils.repeat('-', 35 + this.first.length() + this.second.length()));
