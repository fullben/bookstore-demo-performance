...
public class AddressRepositoryImpl extends BaseRepositoryImpl<AddressEntity> {
  ...
  @Transactional
  @Override
  public void batchInsert(final Collection<AddressEntity> entities) {
    this.batchInsert(
        entities,
        (ps, entity) -> {
  ...
  protected String insertSql() {
    return "INSERT INTO ADDRESS " + "(ID,ADDRESS,ADDRESS2,ZIPCODE,CITY_ID) " + "VALUES(?,?,?,?,?)";
  }

  @Override
  protected String copySql() {
    return "COPY ADDRESS " + "(ID,ADDRESS,ADDRESS2,ZIPCODE,CITY_ID)";
  }
}
