...
public class InventoryItemRepositoryImpl extends BaseRepositoryImpl<InventoryItemEntity> {
  ...
  @Transactional
  @Override
  public void batchInsert(final Collection<InventoryItemEntity> entities) {
    this.batchInsert(
        entities,
        (ps, entity) -> {
  ...
  protected String insertSql() {
    return "INSERT INTO INVENTORYITEM " + "(ID,SHOP_ID,BOOK_ID,AMOUNT) " + "VALUES(?,?,?,?)";
  }

  @Override
  protected String copySql() {
    return "COPY INVENTORYITEM " + "(ID,SHOP_ID,BOOK_ID,AMOUNT)";
  }
}
