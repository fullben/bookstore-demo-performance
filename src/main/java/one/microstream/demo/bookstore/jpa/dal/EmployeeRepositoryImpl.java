...
public class EmployeeRepositoryImpl extends BaseRepositoryImpl<EmployeeEntity> {
  ...
  @Transactional
  @Override
  public void batchInsert(final Collection<EmployeeEntity> entities) {
    this.batchInsert(
        entities,
        (ps, entity) -> {
  ...
  protected String insertSql() {
    return "INSERT INTO EMPLOYEE " + "(ID,NAME,ADDRESS_ID,SHOP_ID) " + "VALUES(?,?,?,?)";
  }

  @Override
  protected String copySql() {
    return "COPY EMPLOYEE " + "(ID,NAME,ADDRESS_ID,SHOP_ID)";
  }
}
