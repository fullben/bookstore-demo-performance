...
public class PurchaseRepositoryImpl extends BaseRepositoryImpl<PurchaseEntity> {
  ...
  @Transactional
  @Override
  public void batchInsert(final Collection<PurchaseEntity> entities) {
    this.batchInsert(
        entities,
        (ps, entity) -> {
          ...
          ps.setTimestamp(
              4, Timestamp.from(entity.getTimestamp().atZone(ZoneId.systemDefault()).toInstant()));
  ...
  protected String copySql() {
    return "COPY PURCHASE " + "(ID,EMPLOYEE_ID,CUSTOMER_ID,TIMESTAMP,SHOP_ID)";
  }
}
