 ...
 *
 * <p>Data amount boundaries can be controlled with {@link RandomDataAmount}.
 */
public class RandomDataGenerator implements HasLogger {
  ...
  private static class CountryData extends ArrayList<City> {
    ...
    CountryData(final Faker faker, final Locale locale) {
      super(512);
  ...
  public DataMetrics generate() {
    ...
    final List<CountryData> countries =
        locales.parallelStream().map(this::createCountry).collect(toList());
    ...
    final DataMetrics metrics =
        new DataMetrics(this.books.bookCount(), countries.size(), this.shops.shopCount());
  ...
  private List<Locale> supportedLocales() {
    ...
    final int max = maxCountries == -1 ? locales.size() : maxCountries;
    return max >= locales.size() ? locales : locales.subList(0, max);
  }
  ...
  private CountryData createCountry(final Locale locale) {
    ...
    final Country country =
        new Country(locale.getDisplayCountry(Locale.ENGLISH), locale.getCountry());
    final CountryData countryData = new CountryData(faker, locale);
    this.randomRange(this.dataAmount.maxCitiesPerCountry())
        .forEach(
            i -> {
              ...
              if (cityNameSet.add(cityName)) {
                ...
                final State state =
                    stateMap.computeIfAbsent(stateName, s -> new State(stateName, country));
                countryData.add(new City(cityName, state));
              }
            });

    countryData.people = new HashMap<>(countryData.size(), 1.0f);
    countryData.parallelStream()
        .forEach(city -> countryData.people.put(city, this.createCustomers(countryData, city)));
    ...
    this.logger()
        .info(
            "+ country "
                ...
                + countryData.size()
                + " cities, "
                + countryData.people.values().stream().mapToInt(List::size).sum()
                + " customers] ");

    return countryData;
  }

  private List<Customer> createCustomers(final CountryData countryData, final City city) {
        ...
        .forEach(
            country -> {
              ...
              final List<Book> books =
                  IntStream.range(0, this.dataAmount.maxBooksPerCountry())
                      .mapToObj(i -> country.faker.book().title())
                      .map(
                          title ->
                              this.createBook(
                                  country, genres, publishers, authors, language, title))
                      .collect(toList());
              ...
              this.logger()
                  .info("+ " + books.size() + " books in " + country.locale.getDisplayCountry());
            });
    ...
    return this.randomRange(this.dataAmount.maxPublishersPerCountry())
        ...
        .map(
            name ->
                new Publisher(
                    name,
                    this.createAddress(countryData.randomCity(this.random), countryData.faker)))
        .collect(toList());
  }
    ...
    countries.parallelStream()
        .forEach(
            country -> {
              ...
              country.forEach(
                  city ->
                      this.randomRange(this.dataAmount.maxShopsPerCity())
                          .forEach(
                              i ->
                                  country.shops.add(this.createShop(countries, country, city, i))));
  ...
  private Shop createShop(
    ...
    final Map<Book, Integer> inventory =
        this.randomRange(this.dataAmount.maxBooksPerShop())
            .mapToObj(i -> this.randomBook())
            .distinct()
            .collect(toMap(book -> book, book -> this.random.nextInt(50) + 1));
  ...
  private void createPurchases(final List<CountryData> countries) {
    ...
    IntStream.rangeClosed(startYear, thisYear)
        .forEach(year -> this.createPurchases(countries, year, customers));
  ...
  private void createPurchases(
      final List<CountryData> countries, final int year, final Set<Customer> customers) {
    ...
    final List<Purchase> purchases =
        countries.parallelStream()
            .flatMap(
                country ->
                    country.shops.stream()
                        .flatMap(shop -> this.createPurchases(country, year, shop)))
            .collect(toList());

    final Set<Customer> customersForYear =
        this.purchases.init(year, purchases, this.storageManager);
  ...
  private Stream<Purchase> createPurchases(
      final CountryData countryData, final int year, final Shop shop) {
    ...
    return shop.employees()
        .flatMap(
            employee ->
                this.randomRange(this.dataAmount.maxPurchasesPerEmployeePerYear())
                    .mapToObj(
                        pi -> {
                          ...
                          final LocalDateTime timestamp =
                              this.randomDateTime(year, isLeapYear, random);
                          final List<PurchaseItem> items =
                              this.randomRange(this.dataAmount.maxItemsPerPurchase())
                                  .mapToObj(
                                      ii ->
                                          new PurchaseItem(
  ...
  private LocalDateTime randomDateTime(
      final int year, final boolean isLeapYear, final Random random) {
  ...
  private List<Employee> createEmployees(final CountryData countryData, final City city) {
  ...
  private Address createAddress(final City city, final Faker faker) {
    final com.github.javafaker.Address fa = faker.address();
    return new Address(fa.streetAddress(), fa.secondaryAddress(), fa.zipCode(), city);
  }

  private BigDecimal randomPurchasePrice() {
    return this.minPrice.add(new BigDecimal(Math.random()).multiply(this.priceRange));
  }
