 ...
 *
 * <p>This type is used to read and write the {@link Book}s, their {@link Author}s, {@link Genre}s,
...
public class Books extends ReadWriteLocked {
   ...
   *
   * <pre>this.add(book, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   * @see #add(Book)
   */
  public void add(final Book book, final Persister persister) {
    this.write(
        () -> {
  ...
  /**
   * Adds a range of new books and stores it with the {@link BookStoreDemo}'s {@link
   * EmbeddedStorageManager}.
   *
   * <p>This is a synonym for:
   *
   * <pre>this.addAll(books, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   */
  public void addAll(final Collection<? extends Book> books, final Persister persister) {
    this.write(
        () -> {
  ...
  /**
   * Adds a book to a map with a list as values. If no list is present for the given key, it will be
   * created.
   *
   ...
   * @param book the book to add
   */
  private <K> void addToMap(final Map<K, List<Book>> map, final K key, final Book book) {
    map.computeIfAbsent(key, k -> new ArrayList<>(1024)).add(book);
  }

  /**
   * Gets all books as a sorted {@link List}. Modifications to the returned list are not reflected
   * to the backed data.
   *
   * @return all books
   */
  public List<Book> all() {
    return this.read(() -> this.isbn13ToBook.values().stream().sorted().collect(toList()));
  }

  /**
   * Gets all authors as a sorted {@link List}. Modifications to the returned list are not reflected
   * to the backed data.
   *
   * @return all authors
   */
  public List<Author> authors() {
    return this.read(() -> this.authorToBooks.keySet().stream().sorted().collect(toList()));
  }

  /**
   * Gets all genres as a sorted {@link List}. Modifications to the returned list are not reflected
   * to the backed data.
   *
   * @return all genres
   */
  public List<Genre> genres() {
    return this.read(() -> this.genreToBooks.keySet().stream().sorted().collect(toList()));
  }

  /**
   * Gets all publishers as a sorted {@link List}. Modifications to the returned list are not
   * reflected to the backed data.
   *
   * @return all publishers
   */
  public List<Publisher> publishers() {
    return this.read(() -> this.publisherToBooks.keySet().stream().sorted().collect(toList()));
  }

  /**
   * Gets all languages as a sorted {@link List}. Modifications to the returned list are not
   * reflected to the backed data.
   *
   * @return all languages
   */
  public List<Language> languages() {
    return this.read(() -> this.languageToBooks.keySet().stream().sorted().collect(toList()));
  }
  ...
  public int bookCount() {
    return this.read(this.isbn13ToBook::size);
  }

  /**
   * Executes a function with a {@link Stream} of {@link Book}s and returns the computed value.
   *
   * <p>This example counts all authors which have written a book with a title starting with 'The':
   *
   * <pre>
  ...
  public <T> T compute(final Function<Stream<Book>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.isbn13ToBook.values().stream()));
  }

  /**
   * Executes a function with a pre-filtered {@link Stream} of {@link Book}s and returns the
   * computed value.
   *
   * @param <T> the return type
  ...
  public <T> T computeByAuthor(
      final Author author, final Function<Stream<Book>, T> streamFunction) {
    return this.read(
        () -> {
   ...
   * @see #compute(Function)
   */
  public <T> T computeByGenre(final Genre genre, final Function<Stream<Book>, T> streamFunction) {
    return this.read(
        () -> {
  ...
  public <T> T computeByPublisher(
      final Publisher publisher, final Function<Stream<Book>, T> streamFunction) {
    return this.read(
        () -> {
  ...
  public <T> T computeByLanguage(
      final Language language, final Function<Stream<Book>, T> streamFunction) {
    return this.read(
        () -> {
  ...
  public Book ofIsbn13(final String isbn13) {
    return this.read(() -> this.isbn13ToBook.get(isbn13));
  }
  ...
  public <T> T computeGenres(final Function<Stream<Genre>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.genreToBooks.keySet().stream()));
  }
  ...
  public <T> T computeAuthors(final Function<Stream<Author>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.authorToBooks.keySet().stream()));
  }
  ...
  public <T> T computePublishers(final Function<Stream<Publisher>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.publisherToBooks.keySet().stream()));
  }
  ...
  public <T> T computeLanguages(final Function<Stream<Language>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.languageToBooks.keySet().stream()));
  }
  ...
  public List<Book> allByAuthor(final Author author) {
    return this.computeByAuthor(author, books -> books.collect(toList()));
  }
  ...
  public List<Book> allByGenre(final Genre genre) {
    return this.computeByGenre(genre, books -> books.collect(toList()));
  }
  ...
  public List<Book> allByPublisher(final Publisher publisher) {
    return this.computeByPublisher(publisher, books -> books.collect(toList()));
  }
  ...
  public List<Book> allByLanguage(final Language language) {
    return this.computeByLanguage(language, books -> books.collect(toList()));
  }
  ...
  private Index<Book> createIndex() {
    final DocumentPopulator<Book> documentPopulator =
        (document, book) -> {
    ...
    final EntityMatcher<Book> entityMatcher =
        document -> this.isbn13ToBook.get(document.get("isbn13"));

    final Index<Book> index = new Index<>(Book.class, documentPopulator, entityMatcher);
