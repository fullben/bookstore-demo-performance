...
public class Inventory extends ReadWriteLocked {
  ...
  public int amount(final Book book) {
    return this.read(() -> coalesce(this.inventoryMap.get(book), 0));
  }
  ...
  public <T> T compute(final Function<Stream<Entry<Book, Integer>>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.inventoryMap.entrySet().stream()));
  }
  ...
  public int slotCount() {
    return this.read(() -> this.inventoryMap.size());
  }

  /**
   * Gets all books and their amount as a {@link List}. Modifications to the returned list are not
   * reflected to the backed data.
   *
   * @return all books and their amount
   */
  public List<Entry<Book, Integer>> slots() {
    return this.read(() -> new ArrayList<>(this.inventoryMap.entrySet()));
  }

  /**
   * Gets all books as a {@link List}. Modifications to the returned list are not reflected to the
   * backed data.
   *
   * @return all books
   */
  public List<Book> books() {
    return this.read(() -> this.inventoryMap.keySet().stream().collect(toList()));
  }
}
