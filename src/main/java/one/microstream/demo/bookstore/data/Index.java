...
public class Index<T> implements Closeable {
   ...
   */
  public synchronized List<T> search(final Query query, final int maxResults) {
    this.lazyInit();
  ...
  private void lazyInit() {
    try {
      if (this.directory == null) {
        final Path path = Paths.get("data", "index", this.entityType.getSimpleName());
        this.directory = new MMapDirectory(path);
        this.writer =
            new IndexWriter(this.directory, new IndexWriterConfig(new StandardAnalyzer()));
        this.searcher = new IndexSearcher(this.reader = DirectoryReader.open(this.writer));
      } else {
