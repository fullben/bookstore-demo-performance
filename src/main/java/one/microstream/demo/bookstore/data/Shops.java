 ...
 *
 * <p>This type is used to read and write the {@link Shop}s, their {@link Employee}s and {@link
 * Inventory}s.
 *
...
public class Shops extends ReadWriteLocked {
   ...
   *
   * <pre>this.add(shop, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   * @see #add(Shop)
   */
  public void add(final Shop shop, final Persister persister) {
    this.write(
        () -> {
          this.shops.add(shop);
  ...
  /**
   * Adds a range of new shops and stores it with the {@link BookStoreDemo}'s {@link
   * EmbeddedStorageManager}.
   *
   * <p>This is a synonym for:
   *
   * <pre>this.addAll(shops, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   */
  public void addAll(final Collection<? extends Shop> shops, final Persister persister) {
    this.write(
        () -> {
  ...
  public int shopCount() {
    return this.read(this.shops::size);
  }

  /**
   * Gets all shops as a {@link List}. Modifications to the returned list are not reflected to the
   * backed data.
   *
   * @return all shops
   */
  public List<Shop> all() {
    return this.read(() -> new ArrayList<>(this.shops));
  }

  /**
   * Clears all {@link Lazy} references used by all shops. This frees the used memory but you do not
   * lose the persisted data. It is loaded again on demand.
   *
   * @see Shop#clear()
   */
  public void clear() {
    this.write(() -> this.shops.forEach(Shop::clear));
  }
  ...
  public <T> T compute(final Function<Stream<Shop>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.shops.parallelStream()));
  }

  /**
   * Executes a function with a {@link Stream} of {@link InventoryItem}s and returns the computed
   * value.
   *
   * @param <T> the return type
   * @return the computed result
                                ...
                                .compute(
                                    entries ->
                                        entries.map(
                                            entry ->
                                                new InventoryItem(
                                                    shop, entry.getKey(), entry.getValue()))))));
  }
  ...
  public Shop ofName(final String name) {
    return this.read(
        () -> this.shops.stream().filter(shop -> shop.name().equals(name)).findAny().orElse(null));
  }
}
