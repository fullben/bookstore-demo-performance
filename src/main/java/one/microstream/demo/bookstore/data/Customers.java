 ...
 *
 * <p>This type is used to read and write the {@link Customer}s.
 *
...
public class Customers extends ReadWriteLocked {
  ...
  /**
   * Adds a new customer and stores it with the {@link BookStoreDemo}'s {@link
   * EmbeddedStorageManager}.
   *
   * <p>This is a synonym for:
   *
   * <pre>this.add(customer, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   * @see #add(Customer)
   */
  public void add(final Customer customer, final Persister persister) {
    this.write(
        () -> {
  ...
  /**
   * Adds a range of new customers and stores it with the {@link BookStoreDemo}'s {@link
   * EmbeddedStorageManager}.
   *
   * <p>This is a synonym for:
   *
   * <pre>this.addAll(customers, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   */
  public void addAll(final Collection<? extends Customer> customers, final Persister persister) {
    this.write(
        () -> {
          this.customers.putAll(
              customers.stream()
                  .collect(Collectors.toMap(Customer::customerId, Function.identity())));
          persister.store(this.customers);
        });
  }
  ...
  public synchronized int customerCount() {
    return this.read(this.customers::size);
  }

  /**
   * Gets all customers as a {@link List}. Modifications to the returned list are not reflected to
   * the backed data.
   *
   * @return all customers
   */
  public List<Customer> all() {
    return this.read(() -> new ArrayList<>(this.customers.values()));
  }
  ...
  public <T> T compute(final Function<Stream<Customer>, T> streamFunction) {
    return this.read(() -> streamFunction.apply(this.customers.values().parallelStream()));
  }
  ...
  public Customer ofId(final int customerId) {
    return this.read(() -> this.customers.get(customerId));
  }
}
