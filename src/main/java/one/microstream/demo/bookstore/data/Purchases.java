 ...
 *
 * <p>This type is used to read and write the {@link Purchase}s and statistics thereof.
 *
 * <p>All operations on this type are thread safe.
 *
 * @see Data#purchases()
 * @see ReadWriteLocked
 */
public class Purchases extends ReadWriteLockedStriped {
   ...
   *
   * <p>Note that this class doesn't need to handle concurrency in any way, since it is only used by
   * the {@link Purchases} implementation which handles thread safety.
   */
  private static class YearlyPurchases {
     ...
     */
    YearlyPurchases add(final Purchase purchase, final Persister persister) {
    ...
    /**
     * Adds a purchase to a map with a list as values. If no list is present for the given key, it
     * will be created.
     *
     ...
     */
    private static <K> void clearMap(final Map<K, Lazy<List<Purchase>>> map) {
      map.values().forEach(lazy -> clearIfStored(lazy).ifPresent(List::clear));
    }
     ...
     */
    Stream<Purchase> byShop(final Shop shop) {
      return ensureParallelStream(Lazy.get(this.shopToPurchases.get(shop)));
    }
     ...
     */
    Stream<Purchase> byShops(final Predicate<Shop> shopSelector) {
     ...
     */
    Stream<Purchase> byEmployee(final Employee employee) {
      return ensureParallelStream(Lazy.get(this.employeeToPurchases.get(employee)));
    }
     ...
     */
    Stream<Purchase> byCustomer(final Customer customer) {
      return ensureParallelStream(Lazy.get(this.customerToPurchases.get(customer)));
    }
  }
  ...
  /**
   * This method is used exclusively by the {@link RandomDataGenerator} and it's not published by
   * the {@link Purchases} interface.
   */
  Set<Customer> init(final int year, final List<Purchase> purchases, final Persister persister) {
    return this.write(
        year,
        () -> {
          ...
          final Set<Customer> customers =
              new HashSet<>(yearlyPurchases.customerToPurchases.keySet());
  ...
  /**
   * Adds a new purchase and stores it with the {@link BookStoreDemo}'s {@link
   * EmbeddedStorageManager}.
   *
   * <p>This is a synonym for:
   *
   * <pre>this.add(purchase, BookStoreDemo.getInstance().storageManager())</pre>
   *
   ...
   * @see #add(Purchase)
   */
  public void add(final Purchase purchase, final Persister persister) {
            ...
            this.write(
                ...
                () -> {
                  this.yearlyPurchases.put(
                      year, Lazy.Reference(new YearlyPurchases().add(purchase, null)));
                  persister.store(this.yearlyPurchases);
                });
          }
        });
    ...
    return this.read(
        ...
        () -> {
          final IntSummaryStatistics summary =
              this.yearlyPurchases.keySet().stream()
  ...
  /**
   * Clears all {@link Lazy} references regarding all purchases. This frees the used memory but you
   * do not lose the persisted data. It is loaded again on demand.
   *
   * @see #clear(int)
   */
  public void clear() {
    final List<Integer> years = this.read(0, () -> new ArrayList<>(this.yearlyPurchases.keySet()));
    years.forEach(this::clear);
  }

  /**
   * Clears all {@link Lazy} references regarding purchases of a specific year. This frees the used
   * memory but you do not lose the persisted data. It is loaded again on demand.
   *
   * @param year the year to clear
   * @see #clear()
   */
  public void clear(final int year) {
    this.write(
        year,
        () -> clearIfStored(this.yearlyPurchases.get(year)).ifPresent(YearlyPurchases::clear));
  }

  /**
   * Executes a function with a pre-filtered {@link Stream} of {@link Purchase}s and returns the
   * computed value.
   *
  ...
  public <T> T computeByShopAndYear(
      final Shop shop, final int year, final Function<Stream<Purchase>, T> streamFunction) {
    return this.read(
        year,
        () -> {
          final YearlyPurchases yearlyPurchases = Lazy.get(this.yearlyPurchases.get(year));
          return streamFunction.apply(
              yearlyPurchases == null ? Stream.empty() : yearlyPurchases.byShop(shop));
        });
  }
  ...
  public <T> T computeByShopsAndYear(
    ...
    return this.read(
        ...
        () -> {
          final YearlyPurchases yearlyPurchases = Lazy.get(this.yearlyPurchases.get(year));
          return streamFunction.apply(
              yearlyPurchases == null ? Stream.empty() : yearlyPurchases.byShops(shopSelector));
        });
  }
  ...
  public <T> T computeByEmployeeAndYear(
      final Employee employee, final int year, final Function<Stream<Purchase>, T> streamFunction) {
    return this.read(
        year,
        () -> {
          final YearlyPurchases yearlyPurchases = Lazy.get(this.yearlyPurchases.get(year));
          return streamFunction.apply(
              yearlyPurchases == null ? Stream.empty() : yearlyPurchases.byEmployee(employee));
        });
  }
  ...
  public <T> T computeByCustomerAndYear(
      final Customer customer, final int year, final Function<Stream<Purchase>, T> streamFunction) {
    return this.read(
        year,
        () -> {
          final YearlyPurchases yearlyPurchases = Lazy.get(this.yearlyPurchases.get(year));
          return streamFunction.apply(
              yearlyPurchases == null ? Stream.empty() : yearlyPurchases.byCustomer(customer));
        });
  }
   ...
   * @return list of best selling books
   */
  public List<BookSales> bestSellerList(final int year, final Country country) {
    return this.computeByShopsAndYear(shopInCountryPredicate(country), year, this::bestSellerList);
  }

  private List<BookSales> bestSellerList(final Stream<Purchase> purchases) {
    return purchases
        .flatMap(Purchase::items)
        .collect(groupingBy(PurchaseItem::book, summingInt(PurchaseItem::amount)))
        .entrySet()
        .stream()
  ...
  public long countPurchasesOfForeigners(final int year) {
    return this.computePurchasesOfForeigners(year, purchases -> purchases.count());
  }
  ...
  public List<Purchase> purchasesOfForeigners(final int year) {
    return this.computePurchasesOfForeigners(year, purchases -> purchases.collect(toList()));
  }

  private <T> T computePurchasesOfForeigners(
      final int year, final Function<Stream<Purchase>, T> streamFunction) {
    return this.computeByYear(
        year, purchases -> streamFunction.apply(purchases.filter(purchaseOfForeignerPredicate())));
  }
   ...
   * @return the amount of computed purchases
   */
  public long countPurchasesOfForeigners(final int year, final Country country) {
    return this.computePurchasesOfForeigners(year, country, purchases -> purchases.count());
  }
  ...
  public List<Purchase> purchasesOfForeigners(final int year, final Country country) {
    return this.computePurchasesOfForeigners(
        year, country, purchases -> purchases.collect(toList()));
  }

  private <T> T computePurchasesOfForeigners(
      final int year, final Country country, final Function<Stream<Purchase>, T> streamFunction) {
    return this.computeByShopsAndYear(
        shopInCountryPredicate(country),
        year,
        purchases -> streamFunction.apply(purchases.filter(purchaseOfForeignerPredicate())));
  }
   ...
   */
  public MonetaryAmount revenueOfShopInYear(final Shop shop, final int year) {
  ...
  public Employee employeeOfTheYear(final int year) {
    return this.computeByYear(year, bestPerformingEmployeeFunction());
  }
  ...
  public Employee employeeOfTheYear(final int year, final Country country) {
    return this.computeByShopsAndYear(
        shopInCountryPredicate(country), year, bestPerformingEmployeeFunction());
  }
    ...
    return purchases ->
        maxKey(
            purchases.collect(
                groupingBy(
                    Purchase::employee,
                    summingMonetaryAmount(BookStoreDemo.CURRENCY_UNIT, Purchase::total))));
  }
}
