...
class Query {
  ...
  public static Query[] All(final Data data, final Repositories repositories) {
    return new Query[] {
  ...
  public static Query AllCustomersPaged(final Data data, final Repositories repositories) {
    return new Query(
        "All customers paged",
        (executor, iterations) -> {
  ...
  public static Query BooksByPrice(final Data data, final Repositories repositories) {
    return new Query(
        "Books by price",
        (executor, iterations) -> {
  ...
  public static Query BestSellerList(final Data data, final Repositories repositories) {
    return new Query(
        "Bestseller list",
        (executor, iterations) ->
            randomYears(data, 3)
                .forEach(
                    year ->
                        randomCountries(data, repositories, 3)
                            .forEach(
  ...
  public static Query EmployeeOfTheYear(final Data data, final Repositories repositories) {
    return new Query(
                                            ...
                                            iteration ->
                                                executor.submit(
                                                    QueryAction.New(
                                                        ...
                                                        () ->
                                                            data.purchases()
                                                                .employeeOfTheYear(year, country),
                                                        () ->
                                                            repositories
                                                                .employeeRepository()
                                                                .employeeOfTheYear(
  ...
  public static Query PurchasesOfForeigners(final Data data, final Repositories repositories) {
    return new Query(
                                            ...
                                            iteration ->
                                                executor.submit(
                                                    QueryAction.New(
                                                        ...
                                                        () ->
                                                            data.purchases()
                                                                .purchasesOfForeigners(
                                                                    year, country),
                                                        () ->
                                                            repositories
                                                                .purchaseRepository()
                                                                .findPurchasesOfForeigners(
                                                                    year, countryEntity)))))));
  }

  public static Query BooksByTitle(final Data data, final Repositories repositories) {
    return new Query(
        "Books by title",
        (executor, iterations) ->
            randomCountries(data, repositories, 3)
                .forEach(
                    (country, countryEntity) ->
                        Arrays.asList("the", "light", "black", "hero", "of")
                            .forEach(
                                pattern ->
                                    IntStream.rangeClosed(1, iterations)
                                        .forEach(
  ...
  public static Query RevenueOfShopInYear(final Data data, final Repositories repositories) {
    return new Query(
        "Revenue of shop",
        (executor, iterations) -> {
          ...
          final ShopEntity shopEntity =
              repositories.shopRepository().findById(shopIndex + 1L).get();
          randomYears(data, 3)
              .forEach(
                  year ->
                      IntStream.rangeClosed(1, iterations)
                                  ...
                                  executor.submit(
                                      QueryAction.New(
                                          ...
                                          () ->
                                              repositories
                                                  .purchaseItemRepository()
                                                  .revenueOfShopInYear(shopEntity, year)))));
        });
  }
  ...
  private static Map<Country, CountryEntity> randomCountries(
      final Data data, final Repositories repositories, final int maxCount) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final CountryEntity countryEntity =
          repositories.countryRepository().findByName(country.name()).get();
  ...
  Query(final String name, final BiConsumer<ActionExecutor, Integer> actionSubmitter) {
    super();
    this.name = name;
