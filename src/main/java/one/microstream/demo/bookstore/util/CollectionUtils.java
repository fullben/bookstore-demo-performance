...
public interface CollectionUtils {
  ...
  /**
   * Returns a {@link Stream} of the given collection or an empty {@link Stream} if the collection
   * is <code>null</code>.
   *
   * @param <T>
   * @param <C>
  ...
  /**
   * Returns a parallel {@link Stream} of the given collection or an empty {@link Stream} if the
  ...
  /**
   * Creates a {@code Collector} that produces the sum of a {@link MonetaryAmount} function applied
   * to the input elements. If no elements are present, the result is {@link MonetaryAmount} of
   * zero.
   *
   ...
   */
  static <T> Collector<T, ?, MonetaryAmount> summingMonetaryAmount(
      final CurrencyUnit currencyUnit, final Function<? super T, MonetaryAmount> mapper) {
    return Collector.of(
